...
class SimpleWindowAPI:
    ...
    def find_window_by_pid(self, pid: int) -> Optional[str]:
        """Finds a window by its process ID using multiple strategies."""
        self.controller.refresh_windows()
        for window_id, window_info in self.controller.window_lookup.items():
            # Strategy 1: Check if PID is embedded in the window_id string (e.g., "HWND_PID_HASH")
            try:
                parts = window_id.split('_')
                if len(parts) > 1 and int(parts[1]) == pid:
                    return window_id
            except (ValueError, IndexError):
                pass  # Not in the expected format, proceed to fallback

            # Strategy 2 (Fallback): Check the process_data dictionary
            if window_info.get('process_data', {}).get('pid') == pid:
                return window_id
        return None
